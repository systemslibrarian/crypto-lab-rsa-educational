 ...
 *
 *  - TEXT OVER A SURFACE AXE DECLINES TO RESOLVE — an `rgba()` over an unknown
 *    backdrop, or a `color-mix()`. Nearly every coloured verdict on this page is
 *    one. `--good-bg`, `--bad-bg`, `--warn-bg` and `--alarm-bg` are all
 *    `rgba(..., 0.1x)` tints, and they carry the four states this lab exists to
 *    show: the keygen `.status.ok` / `.status.bad`, the textbook-RSA
 *    `.status.warn`, the signature `.verdict.good` / `.verdict.bad`, and the
 *    `.verdict.alarm` that fires when the weak key is factored. `.cl-hero-why`,
 *    `.phi-aside`, `.sam-bit.one` and `.sam-row.used` are
 *    `color-mix(in oklab, var(--accent) N%, transparent)`, which Chromium hands
 *    to `getComputedStyle` unresolved. A violations-only gate therefore measured
 *    the contrast of almost none of this lab's generated output.
 *
 *  - TEXT FADED BY AN ANCESTOR'S `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This stylesheet has exactly
 *    three: `.cl-hero-sub` at `.85`, `.btn:disabled` at `.5` (exempt as an
 *    inactive component, and skipped below for that reason), and
 *    `.sam-bit.zero` at `.6` — the 0-bit chips in the square-and-multiply strip,
 *    where the fade is applied to a container whose children are already
 *    `--text-muted`, so the two compound.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. On this page the
 *    clipping box is `.codebox__value` — `overflow-x: auto; white-space: nowrap`
 *    — and EVERY value this lab generates lands in one: the ciphertext, the
 *    recovered plaintext, the digest, the signature, the recovered primes, the
 *    reconstructed private d, and the two 64-hex-char OAEP ciphertexts. At 380px
 *    all of them overflow. Content scrolled past that box is not dimmed or
 ...
 *    and reports a fixed 1:1.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. This lab
 *    has one SVG figure and it is load-bearing: the mod-n clock in Section 3.5,
 *    whose `<text class="clock-label">` marks for m and c are drawn after the
 *    `.clock-dot` circles and the `.clock-chord` lines. An ancestor-only walk
 *    would composite those glyphs onto the `<svg>`'s own transparent background
 *    rather than onto whatever is drawn under them. SVG text also takes its ink
 *    from `fill`, not `color`, which the walk below reads accordingly — reading
 *    `color` there measures an inherited value the glyphs are not painted in.
 *    The `FILLED` guard matters here too: the two chords are `<line>` elements
 *    and SVG's initial `fill` is black, so a naive underlay walk would treat
 *    each chord as an opaque black rectangle covering the whole clock face.
 *
 ...
 * rule. `nontext.ts` covers that class separately; on this page it finds nothing
 * to measure, because `style.css` declares no `content` at all outside the
 * `@media (prefers-reduced-motion: reduce)` block's `*::before, *::after`
 * selector, which sets only durations. The `<details class="trace">` disclosures
 * keep the user agent's own marker rather than replacing it with a glyph.
 *
 * SECOND, `aria-hidden` text that is still painted — see the `ariaHidden` note
 * below, where what this lab hides is enumerated rather than assumed.
 */
 ...
 *
 * The narrowed form exists because of a real gap on this page. SC 1.4.3 is about
 * what a reader SEES, and `aria-hidden` changes only what a reader HEARS, so
 * painted text inside an `aria-hidden` subtree still has to clear 4.5:1 —
 * yet axe skips it and (by default) so does this walk. `.sam-bits`, the binary
 * strip in the square-and-multiply trace, is the one subtree on this page where
 * that gap has teeth: it is `aria-hidden` (correctly — every bit is repeated as
 * prose in the `.sam-row__bit` list below it) and it fades its 0-bits with
 * `opacity`. `scan()` therefore calls this a second time as
 * `auditContrast(page, '.sam-bits *', true)`.
 */
export async function auditContrast(
  ...
  return page.evaluate(([rootSelector, allowAriaHidden]: [string, boolean]) => {
     ...
     *
     * A driven pass here walks seven panels, and the expensive part is the
     * square-and-multiply trace: one `.sam-bit` chip per bit of the exponent
     * (each with two child spans) plus one `.sam-row` per squaring (each with
     * four), all of them siblings re-walking the same ancestors up to `<body>`.
     * The real key exponent turns that into hundreds of elements, and the mod-n
     * clock adds one `<circle class="clock-tick">` for every tick it draws.
     ...
     * that is `.codebox__value` (`overflow-x: auto` around a `white-space:
     * nowrap` run), which is the box every generated value on this page lands
     * in — ciphertext, recovered plaintext, digest, signature, recovered primes,
     * reconstructed d, and the two OAEP ciphertexts — and which overflows for
     * real at 380px.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. This page exercises the guard for real: the mod-n
      // clock's two `<line class="clock-chord">` elements are drawn BEFORE the
      // `<g>` groups that carry the m and c labels, they are stroke-only
      // (`.clock-chord { fill: none }`), and their bounding boxes span the whole
      // clock face. Without the FILLED test a chord would be composited as an
      // opaque black rectangle under every label it crosses. The shared header's
      // hamburger is three more stroke-only <line>s, same shape.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. Here that is the
      // `<details class="trace">` disclosure Section 3 wraps around the
      // square-and-multiply visual — the largest single block of generated
      // markup on the page, and it ships shut. The gate opens it by clicking its
      ...
      // <details> at once, before either of its two scans.
      ...
      //
      // This lab does NOT use that idiom today — `grep -r sr-only src index.html`
      // returns nothing, and its one visually-hidden element (the shared
      // header's `.cl-skip-link`) is parked off the top of the DOCUMENT and is
      // caught by the guard above instead. The check is kept because it costs
      // one computed-style read and because the day a `.sr-only` live region
      // appears — the standard way to announce a verdict change to a screen
      // reader, which this lab's four `role="status"` banners are one refactor
      // away from — every scan would otherwise report a fresh 1.x:1 failure for
      // ink that was never laid down. It is deliberately narrow: only a
     ...
     * lab hides was enumerated rather than assumed, and the one member of that
     * set that carries CHARACTERS is measured for real — see `includeAriaHidden`
     * and the `.sam-bits` call in `gate.ts`'s `scan()`.
     *
     * This lab hides four things:
     *   - `.trapdoor__icon` — the `→` and `⟂` glyphs in Section 1, each sitting
     *     directly above a `<strong>` that says "Easy: multiply" / "Hard: factor"
     *     in the same ink on the same surface;
     *   - `.flow-arrow` — the `→` connectors in the Section 3½ round-trip
     *     diagram, inside a `role="img"` whose `aria-label` narrates the whole
     *     journey;
     *   - `.clock-legend__swatch` — empty colour chips, no text at all;
     *   - `.sam-bits` — the binary strip in the square-and-multiply trace. This
     *     one does carry characters (a `0`/`1` per bit and its index), and it is
     *     the only one that does. Hiding it is correct — every bit it draws is
     *     also written as prose in the `.sam-row__bit` list right below it
     *     ("bit 0 = 1"), which is NOT hidden — but hiding it also removed it from
     *     both oracles while `.sam-bit.zero { opacity: .6 }` was still fading its
     *     ink. Sighted low-vision readers see that fade whatever the
     *     accessibility tree says, so `scan()` measures the strip explicitly.
     *
     * Nothing on this page hides a VALUE. No ciphertext, digest, signature,
     * recovered factor or verdict is inside an `aria-hidden` subtree — which was
     ...
     *
     * This lab has one SVG — the mod-n clock — and it is built with
     * `createElementNS`, so every glyph it draws really is inside a `<text>`.
     * The guard stays because the usual way the mistake arrives is a template
     * literal: interpolating a string ARRAY into one makes JS join it with
     * commas, leaving a run of "," text nodes inside whatever element wraps it —
     * and inside a `<g>` (which is exactly how `marker()` groups each dot with
     * its label) those render nothing, while a walk that measured them would
