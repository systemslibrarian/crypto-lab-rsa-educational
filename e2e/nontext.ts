 ...
 *
 * IN THIS LAB the control-boundary half is the live one. `style.css` defines a
 * `--control-border` token and applies it to `input, select` and to nothing
 * else; every `.btn` on the page — Generate, Roll random primes, the weak-prime
 * button, both square-and-multiply toggles, Replay, Factor it!, Run the OAEP
 * comparison, and every `.codebox__copy` — draws its edge from `--border-color`,
 * which is the SURFACE DIVIDER the panels are outlined with. The spec this gate
 * replaces (`e2e/border.spec.ts`) checked 3:1 on `input:not([type=checkbox]),
 * select` — precisely the two selectors the correct token was already on — and
 * so could only ever confirm itself. The generated-content half finds nothing
 * here: this stylesheet declares no `content` outside the reduced-motion block.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * square-and-multiply trace: one `.sam-bit` chip per exponent bit and one
     * `.sam-row` per squaring, all siblings re-walking the same ancestors up to
     ...
     * measuring the theme. This page's canvas is a flat `var(--bg)` on `<body>`
     * with nothing on `html`, so it exercises the OTHER half of the same
     * machinery: the root paints nothing, and the fallback to `<body>` is what
     * keeps every control here from being judged against a white page. The
     * gradient-aware sampler is borrowed from `contrast.ts` wholesale rather
     * than approximated a second time.
     */
